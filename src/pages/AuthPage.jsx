...
export default function AuthPage({ register=false }) {
  ...
  async function submit(event) {
    ...
    try { await api(`/auth/${register ? 'register' : 'login'}`,{ method:'POST',body:data }); navigate('/dashboard',{ replace:true }); }
  ...
  return <div className="pt-28 pb-16 px-4 min-h-screen bg-[#fffbeb]">
    <div className="max-w-md mx-auto">
      ...
      <h1 className="font-serif text-3xl mb-8">{register ? 'Daftar Admin' : 'Login Admin'}</h1>
      <form onSubmit={submit} className="space-y-5">
        ...
        <label className="block text-sm">Email<input name="email" type="email" required autoComplete="username" className={field} /></label>
        ...
        {register && <><label className="block text-sm">Konfirmasi password<input name="confirmPassword" type="password" required minLength={12} autoComplete="new-password" className={field} /></label><label className="block text-sm">Kode undangan admin<input name="inviteCode" type="password" required autoComplete="off" className={field} /></label></>}
        ...
        <Link className="block text-sm text-[#a67c00] text-center" to={register ? '/login' : '/register'}>{register ? 'Sudah punya akun? Masuk' : 'Daftar akun admin'}</Link>
      </form>
    </div>
  </div>;
}
